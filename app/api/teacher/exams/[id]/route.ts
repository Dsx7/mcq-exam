...
export async function GET(req: NextRequest, { params }: { params: Promise<{ id: string }> }) {
  ...
  const exam = await Exam.findOne({ _id: id, teacherId: session.user.id }).lean();
  if (!exam) return NextResponse.json({ error: 'Not found' }, { status: 404 });

  return NextResponse.json({ exam: { ...(exam as any), _id: (exam as any)._id.toString() } });
}
...
export async function DELETE(req: NextRequest, { params }: { params: Promise<{ id: string }> }) {
  ...
  const { id } = await params;
  await Exam.findOneAndDelete({ _id: id, teacherId: session.user.id, status: 'draft' });
